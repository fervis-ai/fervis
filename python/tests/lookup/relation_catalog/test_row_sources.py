...
def test_api_batch_projection_excludes_non_api_sources() -> None:
    read = _staff_read_for_projection()

    complete = build_row_source_catalog(RelationCatalog(reads=(read,)))
    api_batch = build_api_row_source_catalog(RelationCatalog(reads=(read,)))

    assert CALENDAR_ROW_SOURCE_ID in {source.id for source in complete.sources}
    assert {source.read_id for source in api_batch.sources} == {read.id}


def _staff_read_for_projection() -> EndpointRead:
    return EndpointRead(
        id="list_staff",
        endpoint_name="list_staff",
        resource_names=("staff",),
