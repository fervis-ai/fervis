"""Deterministic ranking of reads for recalled resource-name groups."""
...
def select_resource_name_groups(
    *,
    requested_fact_id: str,
    resource_name_groups: tuple[tuple[str, ...], ...],
    relation_catalog: RelationCatalog,
    max_reads_per_fact: int,
) -> RequestedFactCatalogSelection:
    if not requested_fact_id or max_reads_per_fact < 1:
        raise ValueError("catalog selection requires a fact and positive limit")
    read_facts = _catalog_facts_by_read(relation_catalog)
    selections = tuple(
        resource_name_selection(
            resource_names=tuple(dict.fromkeys(group)),
            relation_catalog=relation_catalog,
            read_facts=read_facts,
        )
        for group in resource_name_groups
        if group
    )
    if not selections:
    ...
    candidate_limit = catalog_candidate_limit(max_reads_per_fact)
    selected_rankings = merge_selected_rankings(
        selections,
        candidate_limit=candidate_limit,
    )
    positive_rankings = merge_positive_rankings(
        tuple(selection.positive_rankings for selection in selections)
    )
    query_terms = _merge_ordered_query_terms(
        tuple(selection.query_terms for selection in selections)
    )
    return RequestedFactCatalogSelection(
        requested_fact_id=requested_fact_id,
...
def catalog_candidate_limit(max_reads_per_fact: int) -> int:
    return max(max_reads_per_fact, MIN_CATALOG_READS_PER_FACT)


@dataclass(frozen=True)
class ResourceNameSelection:
...
def resource_name_selection(
    *,
    ...
    positive_rankings = _round_robin_rankings(positive_groups)
    return ResourceNameSelection(
...
def _merge_exact_rankings(
    selections: tuple[ResourceNameSelection, ...],
    *,
    candidate_limit: int,
...
def unselected_positive_read_ids(
    positive_rankings: tuple[CatalogSelectionRanking, ...],
    *,
