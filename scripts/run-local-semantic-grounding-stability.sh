...
output_dir="$(mktemp -d "${TMPDIR:-/tmp}/fervis-semantic-grounding.XXXXXX")"
boundary_file="$output_dir/boundary.json"
"$repo_root/python/.venv/bin/python" \
  "$repo_root/scripts/experiments/build_semantic_grounding_boundary.py" \
...
exec "$repo_root/python/.venv/bin/python" \
  ...
  --runs "$runs" \
  --workers 1 \
  --assertion-file "$repo_root/scripts/experiments/semantic_grounding_assertion.py" \
