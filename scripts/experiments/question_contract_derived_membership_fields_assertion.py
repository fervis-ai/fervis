"""Validate that only irreducible explicit-predicate decisions remain."""
...
def validate(arguments: dict[str, Any], context: dict[str, Any]) -> list[str]:
    ...
    if any("kind" in test or "test_id" in test for test in tests):
        errors.append("population reauthors backend-derived membership fields")
    input_refs = {
